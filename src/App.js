...
// ── Tooltip for scatter with IC name ─────────────────────────────────────────
const ScatterTooltip = ({active,payload}) => {
  if(!active||!payload||!payload.length) return null;
  const d = payload[0]?.payload;
  if(!d) return null;
  return (
    <div style={{background:"white",border:"1px solid #e2e8f0",borderRadius:8,padding:"8px 12px",fontSize:12,boxShadow:"0 2px 8px rgba(0,0,0,.1)"}}>
      <p style={{margin:"0 0 4px",fontWeight:700,color:"#1e293b"}}>{d.name}</p>
      <p style={{margin:"0 0 2px",color:"#64748b"}}>Output: <b>{d.output}</b></p>
      {d.wer!=null&&<p style={{margin:0,color:"#64748b"}}>WER: <b>{d.wer.toFixed(1)}%</b></p>}
    </div>
  );
};
...
function ModelApp({data,onReload,onManual,onSignOut}) {
  ...
  return (
    <div style={{maxWidth:940,margin:"0 auto",...S.page}}>
      ...
      {tab==="whatif"&&<WhatIfTab data={data} spd={spd} dem={dem} werThresholds={wer} avgAudioMin={avgAudioMin} days={vol.days} baseLp={lp}/>}
...
function WhatIfTab({data,spd,dem,werThresholds,avgAudioMin,days,baseLp}) {
  const {transcribers,reviewers,ers}=data;
  ...
  // Onboarding impact: how many new ICs needed per role to restore baseline util
  const onboardingNeeded = useMemo(()=>{
    return ROLE_META.map(role=>{
      const baseUtil = baseLp.util[role.id];
      const simUtil  = simLP.util[role.id];
      if(simUtil <= baseUtil) return {role, needed:0};
      // avg output per remaining IC
      const remaining = (role.id==="t"?transcribers:role.id==="r"?reviewers:ers).filter((_,i)=>!removed[role.id].includes(i));
      if(!remaining.length) return {role, needed: removed[role.id].length};
      const avgOut = remaining.reduce((s,w)=>s+w.output,0)/remaining.length;
      // supply gap in hours
      const supGap = simLP.demH[role.id] - simLP.supH[role.id];
      const spdVal = role.id==="t"?spd.t:role.id==="r"?spd.r:spd.er;
      const hrsPerIC = role.id==="t" ? (avgOut/days)/spdVal : (avgOut/days)*avgAudioMin/spdVal;
      const needed = hrsPerIC>0 ? Math.ceil(supGap/hrsPerIC) : 0;
      return {role, needed: Math.max(0, needed)};
    });
  }, [simLP, baseLp, removed, transcribers, reviewers, ers, spd, days, avgAudioMin]);

  // TAT comparison data
  const tatCompare = useMemo(()=>TIERS.map(t=>{
    const base = baseLp.tat.find(x=>x.id===t.id);
    const sim  = simLP.tat.find(x=>x.id===t.id);
    return {name:t.label, Baseline: base?+fmt(base.tot):0, Scenario: sim?+fmt(sim.tot):0, target:t.tatTgt};
  }),[simLP,baseLp]);

  // ER rework burden
  const erReworkBurden = useMemo(()=>{
    const removedERs = ers.filter((_,i)=>removed.er.includes(i));
    const removedReviewers = reviewers.filter((_,i)=>removed.r.includes(i));
    const allRemoved = [...removedERs,...removedReviewers];
    if(!allRemoved.length) return null;
    const totalOutputLost = allRemoved.reduce((s,w)=>s+w.output,0);
    const highWerRemoved = allRemoved.filter(w=>w.wer!=null&&w.wer>werThresholds.bad);
    const lowWerRemoved  = allRemoved.filter(w=>w.wer!=null&&w.wer<=werThresholds.great);
    const avgRemovedWer  = avgWer(allRemoved);
    const remainingERs   = ers.filter((_,i)=>!removed.er.includes(i));
    const avgRemainingWer= avgWer(remainingERs);
    const rerateMultiplier = avgRemovedWer!=null&&avgRemovedWer>werThresholds.bad ? 1.4 : 1.1;
    const estimatedReworkHrs = (totalOutputLost * avgAudioMin / 60) * (rerateMultiplier - 1);
    return {totalOutputLost,highWerRemoved,lowWerRemoved,avgRemovedWer,avgRemainingWer,estimatedReworkHrs,rerateMultiplier,allRemoved};
  },[removed,ers,reviewers,werThresholds,avgAudioMin]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 16px",borderRadius:10,background:simLP.feasible?"#dcfce7":"#fee2e2",border:`1px solid ${simLP.feasible?"#bbf7d0":"#fecaca"}`}}>
        <span style={{fontSize:14,fontWeight:600,color:simLP.feasible?"#166534":"#dc2626"}}>{simLP.feasible?"Scenario feasible":"Scenario infeasible"}</span>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {totalRemoved>0&&<span style={{fontSize:12,color:"#64748b"}}>{totalRemoved} removed</span>}
      ...
      <Card>
        <h3 style={S.h3}>Utilization — Baseline vs. Scenario</h3>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
          {ROLE_META.map(role=>{
            const base=baseLp.util[role.id];
            const sim=simLP.util[role.id];
            const delta=Math.round((sim-base)*100);
            const[bg,tc]=sim>.99?["#fee2e2","#dc2626"]:sim>.85?["#fef9c3","#854d0e"]:["#f1f5f9","#475569"];
            return(
              <div key={role.id} style={{padding:"14px",borderRadius:12,background:bg,border:"1px solid #e2e8f0",textAlign:"center"}}>
                <p style={{margin:"0 0 4px",fontSize:11,fontWeight:700,textTransform:"uppercase",color:role.hex}}>{role.label}</p>
                <p style={{margin:"0 0 2px",fontSize:28,fontWeight:700,color:tc}}>{Math.round(sim*100)}%</p>
                <p style={{margin:"0 0 6px",fontSize:11,color:"#94a3b8"}}>baseline: {Math.round(base*100)}%</p>
                <span style={{fontSize:12,fontWeight:700,padding:"2px 8px",borderRadius:999,background:delta>0?"#fee2e2":delta<0?"#dcfce7":"#f1f5f9",color:delta>0?"#dc2626":delta<0?"#166534":"#64748b"}}>
                  {delta>0?`▲ +${delta}pp`:delta<0?`▼ ${delta}pp`:"no change"}
                </span>
              </div>
            );
          })}
        ...
        <div style={{height:200}}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={ROLE_META.map(rm=>({name:rm.label.split(" ")[0],Baseline:Math.round(baseLp.util[rm.id]*100),Scenario:Math.round(simLP.util[rm.id]*100)}))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
              <XAxis dataKey="name" tick={{fontSize:12}}/>
              <YAxis unit="%" domain={[0,120]} tick={{fontSize:11}}/>
              ...
              <Bar dataKey="Scenario" fill="#378ADD" radius={[4,4,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      {totalRemoved>0&&(
        <Card>
          <h3 style={S.h3}>Onboarding impact — ICs needed to restore baseline</h3>
          <p style={{margin:"-8px 0 14px",fontSize:12,color:"#64748b"}}>Estimated based on average output per remaining IC in each role.</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
            {onboardingNeeded.map(({role,needed})=>{
              const rm=ROLE_META.find(r=>r.id===role.id);
              const removedCount=removed[role.id].length;
              return(
                <div key={role.id} style={{padding:"14px",borderRadius:10,background:needed>0?rm.bg:"#f0fdf4",border:`1px solid ${needed>0?rm.hex+"40":"#bbf7d0"}`}}>
                  <p style={{margin:"0 0 6px",fontSize:11,fontWeight:700,color:rm.hex,textTransform:"uppercase"}}>{rm.label}</p>
                  <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:4}}>
                    <span style={{fontSize:32,fontWeight:700,color:needed>0?rm.tx:"#166534"}}>{needed}</span>
                    <span style={{fontSize:13,color:"#64748b"}}>to onboard</span>
                  </div>
                  <p style={{margin:0,fontSize:12,color:"#64748b"}}>{removedCount} removed · {needed>removedCount?"more than removed":needed===0?"no gap":"partial gap"}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* TAT impact */}
      <Card>
        <h3 style={S.h3}>TAT impact by tier</h3>
        <p style={{margin:"-8px 0 14px",fontSize:12,color:"#64748b"}}>Visual bars show how turnaround time shifts under this scenario. Red = over target.</p>
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
          {tatCompare.map(({name,Baseline,Scenario,target})=>{
            const maxVal=Math.max(Baseline,Scenario,target)*1.1;
            const bOver=Baseline>target,sOver=Scenario>target;
            return(
              <div key={name} style={{padding:"12px 14px",borderRadius:10,border:"1px solid #e2e8f0",background:"#fafafa"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  <b style={{fontSize:13}}>{name}</b>
                  <span style={{fontSize:12,color:"#94a3b8"}}>target: {target}h</span>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {[{label:"Baseline",val:Baseline,color:"#94a3b8",over:bOver},{label:"Scenario",val:Scenario,color:sOver?"#ef4444":"#378ADD",over:sOver}].map(({label,val,color,over})=>(
                    <div key={label} style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div style={{flex:1,height:18,borderRadius:4,background:"#f1f5f9",overflow:"hidden",position:"relative"}}>
                        <div style={{height:"100%",borderRadius:4,background:color,width:`${Math.min(val/maxVal*100,100)}%`,transition:"width .4s"}}/>
                        <div style={{position:"absolute",right:0,top:0,height:"100%",width:`${(1-target/maxVal)*100}%`,background:"rgba(239,68,68,.1)",borderLeft:"2px dashed #ef4444"}}/>
                      </div>
                      <span style={{fontSize:12,fontWeight:700,color:over?"#dc2626":"#1e293b",width:40,textAlign:"right"}}>{val}h</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div style={{height:180}}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={tatCompare}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
              <XAxis dataKey="name" tick={{fontSize:11}}/>
              <YAxis tick={{fontSize:11}} label={{value:"hours",angle:-90,position:"insideLeft",fontSize:11,fill:"#94a3b8"}}/>
      ...
      {erReworkBurden&&(
        <Card>
          <h3 style={{...S.h3,color:"#7F77DD"}}>ER rework burden</h3>
          <p style={{margin:"-8px 0 14px",fontSize:12,color:"#64748b"}}>Removing ICs with high WER increases rework load on remaining ERs. Higher WER = more ER correction time per transcript.</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:14}}>
            <div style={{padding:"12px",borderRadius:10,background:"#f5f3ff",border:"1px solid #ddd6fe"}}>
              <p style={{margin:"0 0 4px",fontSize:11,fontWeight:600,color:"#7F77DD",textTransform:"uppercase"}}>Output lost</p>
              <p style={{margin:"0 0 2px",fontSize:24,fontWeight:700,color:"#4c1d95"}}>{erReworkBurden.totalOutputLost}</p>
              <p style={{margin:0,fontSize:12,color:"#64748b"}}>transcripts/reviews across {erReworkBurden.allRemoved.length} removed ICs</p>
            </div>
            <div style={{padding:"12px",borderRadius:10,background:"#fef2f2",border:"1px solid #fecaca"}}>
              <p style={{margin:"0 0 4px",fontSize:11,fontWeight:600,color:"#dc2626",textTransform:"uppercase"}}>Est. rework hours</p>
              <p style={{margin:"0 0 2px",fontSize:24,fontWeight:700,color:"#991b1b"}}>{fmt(erReworkBurden.estimatedReworkHrs)}h</p>
              <p style={{margin:0,fontSize:12,color:"#64748b"}}>{Math.round((erReworkBurden.rerateMultiplier-1)*100)}% rework rate on lost volume</p>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
            <div style={{padding:"10px 12px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
              <p style={{margin:"0 0 2px",fontSize:11,color:"#94a3b8",fontWeight:600,textTransform:"uppercase"}}>Avg WER (removed)</p>
              <p style={{margin:0,fontSize:18,fontWeight:700,color:erReworkBurden.avgRemovedWer!=null&&erReworkBurden.avgRemovedWer>3.5?"#dc2626":"#d97706"}}>{erReworkBurden.avgRemovedWer!=null?`${erReworkBurden.avgRemovedWer.toFixed(1)}%`:"N/A"}</p>
            </div>
            <div style={{padding:"10px 12px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
              <p style={{margin:"0 0 2px",fontSize:11,color:"#94a3b8",fontWeight:600,textTransform:"uppercase"}}>Avg WER (remaining ERs)</p>
              <p style={{margin:0,fontSize:18,fontWeight:700,color:"#166534"}}>{erReworkBurden.avgRemainingWer!=null?`${erReworkBurden.avgRemainingWer.toFixed(1)}%`:"N/A"}</p>
            </div>
            <div style={{padding:"10px 12px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
              <p style={{margin:"0 0 2px",fontSize:11,color:"#94a3b8",fontWeight:600,textTransform:"uppercase"}}>High WER removed</p>
              <p style={{margin:0,fontSize:18,fontWeight:700,color:erReworkBurden.highWerRemoved.length>0?"#dc2626":"#166534"}}>{erReworkBurden.highWerRemoved.length} ICs</p>
            </div>
          </div>
          {erReworkBurden.highWerRemoved.length>0&&(
            <div style={{padding:"10px 14px",borderRadius:8,background:"#fef9c3",border:"1px solid #fde68a",fontSize:12,color:"#92400e"}}>
              ⚠ {erReworkBurden.highWerRemoved.map(w=>w.name).join(", ")} had WER above threshold. Removing them reduces rework burden but also removes capacity — net effect depends on volume coverage.
            </div>
          )}
        </Card>
      )}
...
// Lorenz curve helper
function lorenzPoints(arr) {
  if(!arr.length) return [];
  const sorted=[...arr].sort((a,b)=>a-b);
  const total=sorted.reduce((s,v)=>s+v,0);
  let cumPop=0,cumOut=0;
  const pts=[{x:0,y:0}];
  sorted.forEach(v=>{cumPop++;cumOut+=v;pts.push({x:Math.round(cumPop/sorted.length*100),y:Math.round(cumOut/total*100)});});
  return pts;
}

// ── ENHANCED CONCENTRATION TAB ───────────────────────────────────────────────
function ConcentrationTab({data,werThresholds}) {
  const {transcribers,reviewers,ers}=data;
  const groups=[{label:"Transcribers",hex:"#378ADD",bg:"#eff6ff",tx:"#1e40af",workers:transcribers},{label:"Reviewers",hex:"#1D9E75",bg:"#f0fdf4",tx:"#166534",workers:reviewers},{label:"Exec reviewers",hex:"#7F77DD",bg:"#f5f3ff",tx:"#4c1d95",workers:ers}];
  ...
  // Lorenz data (combined for chart — show one group at a time via selector)
  const [lorenzRole,setLorenzRole]=useState("t");
  const lorenzWorkers=lorenzRole==="t"?transcribers:lorenzRole==="r"?reviewers:ers;
  const lorenzHex=ROLE_META.find(r=>r.id===lorenzRole)?.hex||"#378ADD";
  const lorenzData=lorenzPoints(lorenzWorkers.map(w=>w.output));
  const equalityLine=lorenzData.map(p=>({x:p.x,equality:p.x}));
  const lorenzChartData=lorenzData.map((p,i)=>({...p,equality:equalityLine[i]?.equality??p.x}));
  ...
  const gini=(arr)=>{
    ...
    const num=s.reduce((acc,v,i)=>acc+(2*(i+1)-n-1)*v,0);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* HHI cards with explanation */}
      <Card>
        <h3 style={S.h3}>Herfindahl-Hirschman Index (HHI)</h3>
        <p style={{margin:"-8px 0 14px",fontSize:12,color:"#64748b",lineHeight:1.6}}>
          HHI measures output concentration. It's calculated by squaring each IC's output share (%) and summing across all ICs.
          A score above 2,500 signals high concentration — a few ICs hold most of the capacity, creating single-point-of-failure risk.
          Below 1,500 is considered low/healthy concentration.
        </p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
          {groups.map(({label,hex,bg,tx,workers})=>{
            const h=hhi(workers.map(w=>w.output));
            const g=gini(workers.map(w=>w.output));
            return(
              <div key={label} style={{padding:"16px",borderRadius:12,background:bg,border:`1px solid ${hex}30`,textAlign:"center"}}>
                <p style={{margin:"0 0 2px",fontSize:11,fontWeight:700,color:tx,textTransform:"uppercase"}}>{label}</p>
                <p style={{margin:"0 0 2px",fontSize:36,fontWeight:700,color:hhiColor(h)}}>{h}</p>
                <p style={{margin:"0 0 8px",fontSize:12,fontWeight:600,color:hhiColor(h)}}>{hhiLabel(h)} concentration</p>
                <div style={{padding:"4px 10px",borderRadius:8,background:"white",fontSize:11,color:"#64748b"}}>Gini: <b>{g}</b> · {workers.length} ICs</div>
                <div style={{marginTop:10,height:6,borderRadius:999,background:"rgba(0,0,0,.1)"}}>
                  <div style={{height:"100%",borderRadius:999,background:hhiColor(h),width:`${Math.min(h/5000*100,100)}%`}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:tx,opacity:.7,marginTop:3}}><span>0</span><span>1500</span><span>2500</span><span>5000</span></div>
              </div>
            );
          })}
        </div>
        <div style={{marginTop:14,display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
          {[{range:"< 1,500",label:"Low — healthy distribution",color:"#16a34a",bg:"#dcfce7"},{range:"1,500–2,500",label:"Moderate — some key ICs",color:"#d97706",bg:"#fef9c3"},{range:"> 2,500",label:"High — concentrated risk",color:"#dc2626",bg:"#fee2e2"}].map(({range,label,color,bg})=>(
            <div key={range} style={{padding:"8px 12px",borderRadius:8,background:bg,fontSize:12}}>
              <b style={{color}}>{range}</b><br/><span style={{color:"#64748b"}}>{label}</span>
            </div>
          ))}
        </div>
      </Card>

      {/* Capacity drop chart with explanation */}
      <Card>
        <h3 style={S.h3}>Capacity remaining if top N ICs leave</h3>
        <p style={{margin:"-8px 0 14px",fontSize:12,color:"#64748b",lineHeight:1.6}}>
          Shows what percentage of output capacity remains after removing the highest-output ICs. The steeper the drop, the more concentrated your risk. The red line at 70% marks a critical threshold — below it, the remaining pool likely cannot cover demand.
        </p>
        <div style={{height:220}}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={concChart}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
              <XAxis dataKey="n" tick={{fontSize:11,fill:"#94a3b8"}} label={{value:"Top N removed",position:"insideBottom",offset:-2,fontSize:11,fill:"#94a3b8"}}/>
              <YAxis unit="%" domain={[0,105]} tick={{fontSize:11,fill:"#94a3b8"}}/>
              <Tooltip formatter={v=>`${v}%`} labelFormatter={n=>`Remove top ${n}`}/>
              <Legend wrapperStyle={{fontSize:12}}/>
              <ReferenceLine y={70} stroke="#ef4444" strokeDasharray="5 3" label={{value:"70% critical",position:"insideTopRight",fontSize:10,fill:"#ef4444"}}/>
              <ReferenceLine y={85} stroke="#f59e0b" strokeDasharray="4 3" label={{value:"85% warning",position:"insideTopRight",fontSize:10,fill:"#f59e0b"}}/>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <h3 style={{...S.h3,marginBottom:0}}>Lorenz curve — output inequality</h3>
          <div style={{display:"flex",gap:6}}>
            {ROLE_META.map(rm=>(
              <button key={rm.id} onClick={()=>setLorenzRole(rm.id)} style={{padding:"4px 10px",fontSize:12,borderRadius:8,cursor:"pointer",background:lorenzRole===rm.id?rm.hex:"white",color:lorenzRole===rm.id?"white":"#64748b",border:`1px solid ${lorenzRole===rm.id?rm.hex:"#e2e8f0"}`,fontWeight:lorenzRole===rm.id?600:400}}>
                {rm.label.split(" ")[0]}
              </button>
            ))}
          </div>
        ...
        <p style={{margin:"0 0 14px",fontSize:12,color:"#64748b",lineHeight:1.6}}>
          The Lorenz curve plots cumulative % of ICs (x-axis) vs. cumulative % of output (y-axis). The diagonal line of equality represents perfect distribution — every IC contributes equally. The further the curve bows below the diagonal, the more unequal the output distribution. A large gap means a few ICs are carrying most of the load.
        </p>
        <div style={{height:260}}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={lorenzChartData} margin={{top:10,right:20,bottom:20,left:10}}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/>
              <XAxis dataKey="x" unit="%" tick={{fontSize:11}} label={{value:"Cumulative % of ICs",position:"insideBottom",offset:-8,fontSize:11,fill:"#94a3b8"}}/>
              <YAxis unit="%" domain={[0,100]} tick={{fontSize:11}} label={{value:"Cumulative % of output",angle:-90,position:"insideLeft",fontSize:11,fill:"#94a3b8"}}/>
              <Tooltip formatter={(v,n)=>[`${v}%`,n==="y"?"Output share":"Equality"]}/>
              <Legend wrapperStyle={{fontSize:12}}/>
              <Line type="monotone" dataKey="equality" name="Perfect equality" stroke="#e2e8f0" strokeWidth={1.5} strokeDasharray="6 4" dot={false}/>
              <Line type="monotone" dataKey="y" name="Actual distribution" stroke={lorenzHex} strokeWidth={2.5} dot={false}/>
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div style={{marginTop:10,padding:"10px 14px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0",fontSize:12,color:"#64748b"}}>
          Gini coefficient: <b style={{color:lorenzHex}}>{gini(lorenzWorkers.map(w=>w.output))}</b> — ranges 0 (perfect equality) to 1 (one IC does all work). Above 0.4 suggests meaningful concentration risk.
        </div>
      </Card>

      {/* Scatter plots with IC names on hover */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
        {groups.map(({label,hex,workers})=>{
          const scatterData=workers.map((w,i)=>({...w,x:i+1,output:w.output,name:w.name}));
          return(
            <Card key={label}>
              <Lbl color={hex}>{label} — output scatter</Lbl>
              <p style={{margin:"-4px 0 10px",fontSize:11,color:"#94a3b8"}}>Hover for IC name & WER. Each dot = one IC.</p>
              <div style={{height:180}}>
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{top:5,right:10,bottom:5,left:0}}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/>
                    <XAxis dataKey="x" name="Rank" tick={{fontSize:10}} label={{value:"Rank",position:"insideBottom",offset:-2,fontSize:10,fill:"#94a3b8"}}/>
                    <YAxis dataKey="output" name="Output" tick={{fontSize:10}}/>
                    <ZAxis range={[40,40]}/>
                    <Tooltip content={<ScatterTooltip/>}/>
                    <Scatter data={scatterData} fill={hex} fillOpacity={0.75}/>
                  </ScatterChart>
                </ResponsiveContainer>
              </div>
            </Card>
        ...
        {groups.map(({label,hex,workers})=>{
          ...
          return(
            <Card key={label}>
              ...
              <p style={{margin:"0 0 10px",fontSize:12,color:"#64748b"}}>Top 3 = <b style={{color:top3>60?"#dc2626":"#d97706"}}>{top3}%</b> of output</p>
              <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:10}}>
                {[1,2,3,5].map(n=>{const rem=concDrop(workers.map(w=>w.output),n);return(<div key={n} style={{display:"flex",justifyContent:"space-between",fontSize:12}}><span style={{color:"#64748b"}}>Remove top {n}</span><span style={{fontWeight:600,color:rem<70?"#dc2626":rem<85?"#d97706":"#16a34a"}}>{rem}%</span></div>);})}
              </div>
              {workers.slice(0,5).map((w,i)=>{
                const share=Math.round(w.output/tot*100);
                const t=werTier(w.wer,{great:1.5,bad:3.5});
                return(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:i<4?"1px solid #f1f5f9":"none"}}>
                    <span style={{fontSize:11,color:"#94a3b8",width:16}}>#{i+1}</span>
                    <span style={{flex:1,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{w.name}</span>
                    {w.wer!=null&&<WerDot wer={w.wer} thresholds={{great:1.5,bad:3.5}}/>}
                    <div style={{width:50,height:6,borderRadius:999,background:"#f1f5f9"}}><div style={{height:"100%",borderRadius:999,background:hex,width:`${share}%`}}/></div>
                    <span style={{fontSize:11,color:"#94a3b8",width:28,textAlign:"right"}}>{share}%</span>
                  </div>
                );
              })}
            </Card>
